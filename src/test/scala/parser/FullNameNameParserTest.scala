...
class FullNameNameParserTest extends PropSpec with PropertyChecks with Matchers with EitherValues {
  property("parse english names test") {
    ...
    forAll(names) { (name: String, expected: ParsedName) => {
      NameParser.parse(name).fold(t => fail(t.message), _ should be (expected))
    }}
  }

  property("should give error when empty string in provided") {
    NameParser.parse("").fold(
      _ => succeed,
      _ => fail()
    )
  }
