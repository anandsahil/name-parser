...
class ParserHelpersTest extends FunSuite with Matchers {
  ...
  test("split matching suffixes helper test") {
    ...
    val (np, suffixes, nc) = Parse.splitMatchingSuffixes(Suffix(English)) tupled namesParsed
  ...
  test("split name and comma parts with suffix helper test") {
    ...
    Parse.splitMatchingSuffixes(Suffix(English)) tupled Parse.splitNamesAndCommas(nameWithComma.split(" ").toList) should be(
  ...
  test ("conjugation names parsing helper test") {
    ...
    val prefixedName = Parse.joinNamePrefixes(Prefix(English)) tupled namesParsed

    prefixedName should be (
      List("Jüan", "Martinez", "de Lorenzo", "y", "Gutierez"), List.fill(6)(None)
    )

    Parse.joinConjugationToSurroundingNames(Conjungation(English)) tupled prefixedName should be (List("Jüan", "Martinez", "de Lorenzo y Gutierez"), List.fill(4)(None))

    Parse.joinConjugationToSurroundingNames(Conjungation(English)) tupled (
  ...
  test ("conjugation names reverse parsing helper test") {
    ...
    Parse.joinConjugationToSurroundingNames(Conjungation(English)) tupled prefixedName should be (
  ...
  test ("unknown suffix with wong commas placement helper test") {
    ...
    val (np, suffixes, nc) = Parse.splitMatchingSuffixes(Suffix(English)) tupled Parse.splitNamesAndCommas(nameWithComma.split(" ").toList)
