...
object NameParser {
  ...
  private[this] val commaSymbol = ','

  def parse(fullName: String, culture: Culture = English): ParsingResult = {
  ...
  private[parser] def removeNickNameFromFullName(nickName: Option[String], fullName: String): String =
    nickName.map(r => (" " + fullName + " ").replace(r, " ").trim).getOrElse(fullName)

  private[parser] def removeNickNameSymbols(nickName: String): String = {
  ...
  private[parser] def fetchNickName(fullName: String): \/[ParsingFailure, Option[String]] = {
  ...
  private[parser] def adjustComma(commas: List[Option[Char]], index: Int): List[Option[Char]] = {
    if (commas.lift(index).join[Char].map(_ === ',').exists(identity)) commas.patch(index + 1, Nil, 1) else commas.patch(index, Nil, 1)
  }

  private[parser] def splitNamesAndCommas(nameParts: List[String]): (List[String], List[Option[Char]]) = {
  ...
  private[parser] def splitMatchingSuffixes(suffixes: Suffix): (List[String], List[Option[Char]]) => (List[String], List[String], List[Option[Char]]) =
  ...
  private[parser] def splitMatchingTitles(titles: Titles): (List[String], List[Option[Char]]) => (List[String], List[String], List[Option[Char]]) =
    (nameParts: List[String], nameCommas: List[Option[Char]]) => {
  ...
  private[parser] def joinNamePrefixes(prefixes: Prefix): (List[String], List[Option[Char]]) => (List[String], List[Option[Char]]) = (nameParts: List[String], nameCommas: List[Option[Char]]) => {
  ...
  private[parser] def joinConjugationToSurroundingNames(conjugations: Conjungation): (List[String], List[Option[Char]]) => (List[String], List[Option[Char]]) =
    (nameParts: List[String], nameCommas: List[Option[Char]]) => {
      val npRev = nameParts.reverse
  ...
  private[parser] def gatherSuffixLeft(nameParts: List[String], nameCommas: List[Option[Char]]): (List[String], List[Option[Char]], List[String]) = {
  ...
  private[parser] def extractFirstNameLastNameMiddleName(nameParts: List[String], nameCommas: List[Option[Char]]): (Option[String], Option[String], Option[String]) = {
